...
class FactFindForm(http.Controller):
    ...
    def fact_find_form(self, fact_find_id=None, **kwargs):
    ...
    @http.route(['/fact_find/form/submit'], type='http', methods=['GET', 'POST'], auth="user", website=True)
    def fact_find_form_submit(self, **kw):
        if request.httprequest.method == 'POST':
            values = request.params.copy()
            fact_find_forms = request.env['fact.find'].with_user(SUPERUSER_ID).browse(int(values['fact_find_id'])).exists()
            if fact_find_forms:
                if request.env.user.partner_id == fact_find_forms.lead_id.partner_id or request.env.user.partner_id == fact_find_forms.partner_id:
                    ...
                    fact_find_forms.sudo().write({
                        ...
                        'other_email_address': values.get('other_email_address', False),
                    ...
                    fact_find_forms.lead_id.send_kyc_notification()
    ...
    @http.route('/fact_find/document/submit', type='http', auth="user", website=True)
    def fact_find_form_document_submit(self, **kw):
        if request.httprequest.method == 'POST':
            values = request.params.copy()
            fact_find_form_id = int(values.get('fact_find_id'))
            fact_find_form = request.env['fact.find'].sudo().browse(fact_find_form_id)
            if fact_find_form and (
